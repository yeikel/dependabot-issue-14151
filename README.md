...
This project sets `min-release-age=30` in `.npmrc`, telling npm to refuse
installing any package version published less than 30 days ago. Dependabot
...
cooldown, proposing only versions that are at least 30 days old.

## Actual behavior

Dependabot opens a pull request updating `etag` to the latest version even
when that version was published within the 30-day window. Running
`npm install` after merging such a PR fails with:

```
npm error code ETARGET
npm error notarget No matching version found for etag@...
...
2. Dependabot opens a PR proposing `etag` be updated to the latest version.
3. Merge the PR and run `npm install`.
4. npm rejects the install because the proposed version is younger than
   the 30-day `min-release-age` threshold.
